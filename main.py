...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://mitmedialab.github.io/doodlebot-test-ui/", "http://localhost:5173", "http://localhost:8602", "http://doodlebot.media.mit.edu", "https://playground.raise.mit.edu"],  # Allows all origins
...
class VoiceAssistant:
    ...
    async def get_chat_response(self, text: str, system_prompt: str) -> str:
...
async def prompt(request: Request):
    data = await request.json()
    text_input = data.get("text_input")
    system_prompt = data.get("system_prompt")  # 👈 new line

    assistant = VoiceAssistant()
    try:
        response_text = await assistant.get_chat_response(text_input, system_prompt)
